...
class Actor(object):
    def __init__(self, categories, cat_vec_len=20, hidden_sz=50, seq_len=30, dropout=0.1):
        ...
        self._moving_avg = np.zeros((seq_len,))
        self._avg_counts = np.zeros((seq_len,))
    ...
    def build_network(self, categories, cat_vec_len, hidden_sz, max_seq_len, dropout):
        ...
        self.answ_correct = tf.placeholder(tf.int32, (None, max_seq_len))
        self.seq_len = tf.placeholder(tf.int32, (None))
        with tf.variable_scope("Actor"):
            embed = tf.get_variable("embed", (categories * 2, cat_vec_len), dtype=tf.float32
                                    , initializer=xav_init)
            # embed = tf.get_variable("embed",  dtype=tf.float32
            #                        , initializer=np.identity(categories * 2, dtype=np.float32))
            q_embed = tf.nn.embedding_lookup(embed, self.q + categories * self.answ_correct)
    ...
    def apply_grad(self, session, action_perf):
        ...
        s = self.seq_lens[0] - 1
        assert np.all(s + 1 == self.seq_lens)

        self._moving_avg[s] = (self._moving_avg[s] * self._avg_counts[s] + np.sum(action_perf)) / \
                           (self._avg_counts[s] + len(action_perf))
        self._avg_counts[s] += len(action_perf)
        ...
        action_delta = action_perf - self._moving_avg[s]
